...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (err) throw err;
      setMessage(
        'Check your email for a link to reset your password. The link will expire in an hour.'
      );
    } catch (err) {
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-orange-100">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-500 mt-2 text-center">
            Enter the email you use for PantryChef and we&apos;ll send you a link to reset your
            password.
          </p>
        </div>
