...
export default function ResetPasswordPage({ recoveryFlagKey }: ResetPasswordPageProps) {
  ...
  const handleUpdatePassword = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!session) {
        setError(
          'Your reset link may have expired or was already used. Please request a new password reset link.'
        );
        setLoading(false);
        return;
      }
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-orange-100">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-gray-500 mt-2 text-center">
            Choose a strong password for your PantryChef account. You&apos;ll use it to sign in from
            now on.
          </p>
        </div>
